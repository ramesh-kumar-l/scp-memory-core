# 27 — Active Initiatives

**Status:** Active · **Last updated:** 2026-06-20

Cross-cutting initiatives currently in flight. Distinct from the single active
*phase* ([08](08-active-phase.md)); initiatives may span or precede phases.

## In Flight

| Initiative | Status | Notes |
|---|---|---|
| — | — | No active initiative; awaiting approval to start Phase 6. |

## Queued (not started)

- Phase 6 — Observability (pending explicit approval to begin)

## Done

- **Phase 5 — SDKs** (2026-06-20): Python (`scp-memory-sdk`) + TypeScript
  (`@scp/memory-sdk`) clients over the full API incl. trust; offline local embedder
  (sentence-transformers, ADR-011) behind the `Embedder` seam; 96 Python + 6 TS tests.

- **Phase 0 — Project Foundation** (2026-06-20): full memory bank + locked stack.
- **Phase 1 — Memory Core** (2026-06-20): audited, namespaced CRUD; 18 tests; CI.
- **Phase 2 — Memory Intelligence** (2026-06-20): importance, dedup, consolidation,
  decay; first `memory_relations` writes; 46 tests.
- **Phase 3 — Hybrid Retrieval** (2026-06-20): keyword (BM25) + vector + metadata +
  explainable ranking fusion; pluggable `Embedder`/vector backends; 70 tests.
- **Phase 4 — Trust Layer** (2026-06-20): provenance quality, confidence
  (corroboration/contradiction), type-aware freshness, explainability; trust folded
  into ranking + explain endpoint; 86 tests.

## Related

[08-active-phase](08-active-phase.md) · [09-backlog](09-backlog.md) · [23-roadmap](23-roadmap.md)
